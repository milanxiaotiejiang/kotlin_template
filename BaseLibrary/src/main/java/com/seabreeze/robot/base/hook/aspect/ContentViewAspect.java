 ...
 * Des: 需要重写 setContentView
 * 记录 setContentView 加载时间，如果较长可用 X2C 或 异步进行优化
 */
@Aspect
    ...
    public Object activityContentViewAround(ProceedingJoinPoint proceedingJoinPoint) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(activityName) && !TextUtils.isEmpty(sign) && sign
                ...
                XLog.d(String.format("AOP setContentView：%s, %s方法执行了，用时%d ms", activityName, methodName, duration));
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
