...
import com.seabreeze.robot.base.utils.TaskSortUtil;
...
import com.seabreeze.robot.base.launchstarter.task.TaskCallBack;
import com.seabreeze.robot.base.utils.Utils;
...
public class TaskDispatcher {
    ...
    public void start() {
        ...
        if (mAllTasks.size() > 0) {
            ...
            XLog.i("task analyse cost " + (System.currentTimeMillis() - mStartTime) + "  begin main ");
            executeTaskMain();
        }
        XLog.i("task analyse cost startTime cost " + (System.currentTimeMillis() - mStartTime));
    }
    ...
    private void executeTaskMain() {
        ...
        for (Task task : mMainThreadTasks) {
            ...
            XLog.i("real main " + task.getClass().getSimpleName() + " cost   " + (System.currentTimeMillis() - time));
        }
        XLog.i("maintask cost " + (System.currentTimeMillis() - mStartTime));
    }
    ...
    private void printDependedMsg() {
        XLog.i("needWait size : " + (mNeedWaitCount.get()));
        if (false) {
            for (Class<? extends Task> cls : mDependedHashMap.keySet()) {
                XLog.i("cls " + cls.getSimpleName() + "   " + mDependedHashMap.get(cls).size());
                for (Task task : mDependedHashMap.get(cls)) {
                    XLog.i("cls       " + task.getClass().getSimpleName());
                }
            }
        }
    }
            ...
            if (task.needCall()) {
                task.setTaskCallBack(new TaskCallBack() {
                    ...
                    public void call() {
                        ...
                        markTaskDone(task);
                        XLog.i(task.getClass().getSimpleName() + " finish");
    ...
    public void await() {
        try {
            XLog.i("still has " + mNeedWaitCount.get());
            for (Task task : mNeedWaitTasks) {
                XLog.i("needWait: " + task.getClass().getSimpleName());
            }
